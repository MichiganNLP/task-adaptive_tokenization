...
class GenerationArguments:
    ...
    exponential_decay_length_penalty: Optional[str] = field(
        default=None, metadata={"help": "exponential_decay_length_penalty"}
    ) 
...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        #     batch["labels"] = [
        #         [(l if l != self.tokenizer.pad_token_id else torch.tensor(-100, dtype = torch.long, device = l.device)) for l in label] for label in batch["labels"]
        #     ]
        return batch
...
def main():
    ...
    def input_function(example):
        with CaptureLogger(tok_logger) as cl:  
            que_text ="[QUE] " + example["question"].replace("\n",bi_tokenizer.source_tokenizer.sep_token) 
            ...
            if  model_args.model_type == "customed":
                kwargs.update({"tokenize_with_sampling":training_args.do_train}) 
                kwargs.update({"tokenize_with_sampling":True}) 
    ...
    def mhp_input_function(example):
        with CaptureLogger(tok_logger) as cl:  
            que_text ="[QUE] " + example["question"].replace("\n",bi_tokenizer.source_tokenizer.sep_token) 
            ans_text = "[ANS] " + example["comment"].replace("\n",bi_tokenizer.source_tokenizer.sep_token) 
            
            example["input_text"] = que_text + ans_text
            example["ans_text"] = ans_text
            ...
            def fuc(cur):
                temp = temp_res["que_token_res"][cur][:511]
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        lm_datasets = raw_datasets.map(
            ...
            remove_columns=list(filter(lambda x:x!="reference",column_names)),
            load_from_cache_file=not data_args.overwrite_cache,
            desc="Running posprocess on dataset",
        )
        
#     logger.info(f"dataset average input length: {round(sum(total_length)/len(total_length),2)}")

    # with training_args.main_process_first(desc="dataset map tokenization"):
    #     tokenized_datasets = pos_datasets.map(
    #         tokenize_function,
    #         batched=True,
    #         num_proc=data_args.preprocessing_num_workers,
    #         load_from_cache_file=not data_args.overwrite_cache,
    ...
    data_collator_padding = DataCollatorWithPadding(max_length=1024, tokenizer=bi_tokenizer.target_tokenizer, padding="longest")
    ...
    if generation_args.do_generation_test:
        ...
        for inputs in test_data_loader:
            ...
            oracle_text = (inputs["input_text"][0].split("[ANS]")[1].replace(bi_tokenizer.target_tokenizer.sep_token, " ").replace(bi_tokenizer.target_tokenizer.cls_token, "")).strip()
            encoded_prompt_ids = bi_tokenizer.source_tokenizer.encode(
                prompt_text, add_special_tokens=False
            )
            encoded_prompt = [encoded_prompt_ids[:511] + bi_tokenizer.source_tokenizer.encode("[ANS]",  add_special_tokens=False)]
            ...
            for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
                ...
                if data_args.dataset_config_name=="w strategy":           
                    generated_sequence= list(filter(lambda x: x < 21132 or x > 21138, generated_sequence))     
                # print(generated_sequence)     
                # Decode text
                text = bi_tokenizer.target_tokenizer.decode(generated_sequence, clean_up_tokenization_spaces=True)

                # # Remove all text after the stop token
                text = text[: text.find(bi_tokenizer.target_tokenizer.cls_token)]
                # print(type(model))
                # print(generated_sequence)
                ...
                generated_response = text[len(bi_tokenizer.target_tokenizer.decode(encoded_prompt[0], clean_up_tokenization_spaces=True)) :].replace(bi_tokenizer.target_tokenizer.sep_token, " ")             
                generated_response = generated_response.replace(" ", "")
                logger.info(len(generated_response))
                # generated_sequences.append(total_sequence)
                results.append({"context": prompt_text, "generated": generated_response, "tokens": text_tokens, "oracle": oracle_text})
                # predictions.append(" ".join(list(generated_response)))
                # references.append(" ".join(list(oracle_text)))
                logger.info(generated_response)
                
                if prompt_text in ref_dict:
                    
                    pred_dict[prompt_text].append(" ".join(list(generated_response)))
                    ref_dict[prompt_text].add(" ".join(list(oracle_text)))
                else:
                    ...
                    pred_dict[prompt_text].append(" ".join(list(generated_response)))
                    ref_dict[prompt_text].add(" ".join(list(oracle_text)))
                # ref_dict[prompt_text] = set(" ".join(list(inp)) for inp in inputs["reference"][0])  
                
                # print(ref_dict[prompt_text])                
                # logger.info(f"{len(ref_dict[prompt_text])}, {len(pred_dict[prompt_text])}")
        ...
        print(rouge_res)
        import json
        suffix = (training_args.output_dir).split("/")[-1] if (training_args.output_dir).split("/")[-1] else (training_args.output_dir).split("/")[-2]
        json.dump(results, open(f"evaluate_{suffix}_k{generation_args.k}_t{generation_args.temperature}_p{generation_args.p}_r{generation_args.repetition_penalty}_exp{generation_args.exponential_decay_length_penalty[0]}_{generation_args.exponential_decay_length_penalty[1]*100}_beam.out", "w"), indent=2, ensure_ascii=False) 
